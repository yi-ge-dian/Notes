...
不同的dbms管理磁盘文件中的页面 不同的方式。&#x20;

→堆文件组织（Heap File Organization）

→树文件组织 （Tree File Organization）

→顺序/排序文件组织(ISAM) （Sequential / Sorted File Organization (ISAM)）

→哈希文件组织 （→ Hashing File Organization）

这些文件组织方式在不同的DBMS中实现方式不同，因此会有不同的优缺点。在这段文字中，作者强调了我们不需要关心这些数据页内部的具体内容，而是应该理解不同的文件组织方式是如何影响DBMS对数据的处理效率的

1. 堆文件组织（Heap File Organization）：将数据页简单地组织为一个连续的数据块，数据页的排列顺序并不重要，因此对数据的查询、插入和删除操作的效率相对较低。
2. 树形文件组织（Tree File Organization）：使用一种树形结构（如B树或B+树）来组织数据页，通过树的索引结构可以快速定位到需要的数据页，从而提高了对数据的查询和插入操作的效率。
3. 顺序/排序文件组织（Sequential/Sorted File Organization，又称ISAM）：将数据页按照一定的顺序进行组织，如按照主键或索引值进行排序，这样可以使得查询操作具有更高的效率。
4. 散列文件组织（Hashing File Organization）：使用散列函数将数据页散列到不同的磁盘块中，可以提高对数据的查询操作的效率，但是对于数据的插入和删除操作比较困难。

#### Heap File

堆文件是页面的无序集合，其中元组按随机顺序存储。&#x20;

→Create / Get / Write / Delete Page

→还必须支持遍历所有页面。&#x20;

如果只有一个文件，那么很容易找到页面。直接 page id \* page\_size

但是如果有多个文件，则需要元数据来跟踪不同文件中存在的数据页以及哪些数据页具有可用的空间。这些元数据记录了数据页的一些信息，例如文件名、数据页编号、数据页是否为空等等。



目录页记录了数据页在数据库文件中的位置信息，它们可以帮助DBMS快速定位到需要的数据页。

同时，DBMS还需要确保目录页与数据页的同步。在插入、删除、更新数据时，目录页需要及时地记录和更新数据页的位置和状态，以便DBMS能够正确地处理和维护数据。

除此之外，目录页还记录了关于可用空间的元数据，包括：

1. 每个数据页中可用空间的剩余数量（即空闲槽位数量）。
2. 空闲页（即没有被使用的数据页）的列表。

这些信息可以帮助DBMS快速找到可用的空间，从而实现对数据的高效管理和处理。
